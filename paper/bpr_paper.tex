...
(scalar/Abelian) particle sector: the boundary content provably
cannot host a light parity-odd state, and the predicted
$2^{++}/0^{++}$ ratio fell outside the locked pass band
...
non-Abelian particle sector (Postulate~0d, gauged dihedral point
group; \texttt{doc/BPR2\_PATH\_B\_NONABELIAN\_GAUGE.md}).  The
flavor-sector predictions reported here are numerically unchanged
under the proposed gauging (verified exactly).  All closed negative
results are indexed in \texttt{doc/CLOSED\_AND\_DEPRECATED.md}.
\end{quote}
...
\begin{itemize}
\item $p = 104{,}761$ is the nearest prime to
  $p_\text{exact}=104{,}749$ satisfying $p\equiv 1\pmod{4}$, derived
  by inverting the boundary formula for $1/\alphaEM$; the resulting
  value agrees with CODATA to within $0.01\%$;
...
$p_\text{exact}=104{,}749$ matching $\alphaEM$; whether this constitutes a ``prediction'' or a
