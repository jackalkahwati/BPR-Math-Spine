...
>
> **TOE CONNECTION CAMPAIGN (2026-09-13):** [six-module scope and ledger](doc/derivations/toe_connection_campaign_2026-09-13.md). The first [occupation-orbit gauge-encoding calculation](doc/derivations/substrate_gauge_encoding_2026-09-13.md) reproduces a regular-representation algebra but not closed Bose dynamics. The fixed five-site encoding has only a scalar compressed Hamiltonian, nonzero leakage for every encoded state, and a strictly positive short-time mismatch from the supplied D5 target. This excludes that candidate, not all emergent gauge mechanisms. Verification, the repaired subnormal-arithmetic defect and inherited regression limitations are recorded separately; no local gauge emergence or empirical validation is claimed.
>
> **QUANTUM PROPAGATION (2026-09-13):** [density-commutator support and finite-sector bounds](doc/derivations/substrate_quantum_propagation_2026-09-13.md). The complete Bose ring has exact nested-commutator support cancellation below graph distance and a conservative finite-number propagation envelope. Independent Fourier-lift and matrix-series checks cover the fixed controls;232 focused tests and47 independent probes passed after a test-only counter-wiring repair, with production unchanged. All108 observations are available, but28 bounds saturate the stipulated cap. A finite-sector bound is not a thermodynamic velocity, causal cone or emergent relativistic spacetime; numerical and inherited regression limitations remain explicit.
>
> **CHIRAL SOURCE COMPATIBILITY (2026-09-13):** [internal zero-subspace shifts and leakage](doc/derivations/chiral_source_compatibility_2026-09-13.md). On the supplied q=3 sphere, nonconstant scalar sources leak from the original zero-mode subspace, while unitary conjugation preserves the internal Dirac index. Additive sources instead produce first-order shifts; the formal second-order term vanishes by chirality and is not the positive leakage Gram. All6 cases and18 phase records are available, with raw negative roundoff eigenvalues retained;232 focused tests passed. Static repair, probe-launch interruption and regression status are recorded separately. This does not derive physical spacetime chirality or Standard Model matter.
>
> **MIXED SOURCE INTEGRABILITY (2026-09-13):** [external-source contacts and consistency checks](doc/derivations/substrate_source_integrability_2026-09-13.md). All36 fixed cases satisfy the checked operator and contact-completed Ward identities within preregistered heuristic thresholds;24 directional finite-difference comparisons agree, and removing required contacts produces the predicted failures. Focused batched verification passed325 tests with4 platform skips;1275 independent assertions, isolated demos and8 algebra groups passed. Memory-interrupted attempts and pre-execution repairs remain recorded. All600 entry-local arithmetic-unresolved flags are retained. Full33-suite verification completed4424 passes/7 platform skips through manifest-complete resumed fresh-process coverage, not an uninterrupted combined run. This diagnostic source family supplies neither dynamical gravity nor empirical validation.
>
> **EXTENSIVE STABILITY (2026-09-13):** [all-population energy bounds and fixed-density envelopes](doc/derivations/substrate_extensive_stability_2026-09-13.md). Positive repulsion gives a lower energy bound proportional to site count across all populations; exact fixed-density bounds retain the inherited named trial states. All9 small dense controls agree with their bounds under heuristic screens, and18 large-volume controls use exact scalar arithmetic without basis allocation. Resumed focused verification completed348 passes/1 platform skip; isolated demos/eight algebra groups passed. A single bounded same-process check reproduced the historical one-ULP failure (1092 passes/1 failure); it was not retried or relabeled passing. Full34-suite fresh-process regression completed4772 passes/8 platform skips across4780 unique nodes in114 groups; this does not erase the failed same-process check. These bounds establish neither a thermodynamic limit, a selected vacuum, gravity/cosmology nor empirical validation.
>
