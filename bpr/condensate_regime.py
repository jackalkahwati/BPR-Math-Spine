...
(4) THE REGIME VERDICT — the geometry decides, and it is favorable. The
    ...
    T* = N / sum'(1/eps_k):
      ...
      * 2D S^2 boundary (the physical case): T* = C n / ln p  (verified:
        ...
        audit.)
    So IF the boundary dispersion, cutoff and density are as assumed, the 2D
    condition is T_substrate below ~10% of the natural scale. Whether that
    holds is undetermined (see scope note below).
...
SCOPE OF THE THEOREM (2026-09-10 review; supersedes earlier "GUARANTEED /
ROBUST" wording)
------------------------------------------------------------------------
* (1) is a CLASSICAL, FIXED-NORM statement about the frozen ring
  Hamiltonian. It does not show that the physical QUANTUM vacuum of the
  interacting substrate is that condensate; that requires the quantum
  many-body ground state, which is not computed here.
* (3)-(4) use Rayleigh-Jeans (classical, weak-coupling) occupation on an
  ASSUMED dispersion, cutoff l~sqrt(p), and unit density. T* ~ 0.086 C is
  conditional on every one of those choices; it is not an exact property
  of the interacting quantum system.
* The substrate temperature is not derived. A condition "T_substrate < T*"
  cannot be evaluated, so coherence is NOT established, only conditioned.

NET: the vacuum of the frozen CLASSICAL ring model at fixed norm is the
uniform condensate (exact). Physical coherence is CONDITIONAL on (i) the
quantum vacuum tracking the classical minimum, (ii) the assumed 2D
dispersion/cutoff/density, and (iii) an undetermined substrate temperature
below ~C/ln p. Relaxation can further be obstructed by metastable windings.
"""
...
def regime_verdict(p: int = P_SUBSTRATE) -> dict:
    ...
    ts = t_star_sphere(p)
    return {
        "vacuum_is_condensate": "EXACT for the classical fixed-norm ring model; "
                                "quantum vacuum NOT computed",
        ...
        "t_star_sphere_2d": ts,
        "sphere_scaling": "T* ~ C/ln p — conditional on assumed dispersion, "
                          "cutoff l~sqrt(p), unit density, RJ weak coupling",
        "ln_p": ln_p,
        "coherence_condition": f"CONDITIONAL: T_substrate below ~{ts:.0%} of the "
                               "natural scale, under the model assumptions above",
        "undetermined": "the substrate's ambient temperature; whether the quantum "
                        "vacuum tracks the classical minimum (assumption); the "
                        "physical dispersion, cutoff and density entering T*",
        "status": "CONDITIONAL — coherence not established, only conditioned",
    }
...
def report(p: int = P_SUBSTRATE) -> str:
    ...
    lines = [
        ...
        "==========================================================",
        "",
        "(1) Ground-state theorem (exact, CLASSICAL fixed-norm ring model):",
        f"    winding-free flow: n0 = {gs['windingfree_n0']:.6f} at "
        f"H = {gs['windingfree_H']:.4f} (uniform: {gs['H_uniform']:.4f})",
        "    -> the classical minimum of the frozen model is the uniform",
        "       condensate. The QUANTUM vacuum is not computed here.",
        "",
        ...
        "",
        "(3) Equilibrium condensation curve (Rayleigh-Jeans, classical weak coupling):",
        ...
        "",
        "(4) Characteristic temperature (conditional on assumed dispersion/cutoff/density):",
        ...
        "",
        "NET: CONDITIONAL. The classical fixed-norm minimum is the condensate",
        "(exact). Physical coherence requires (i) the quantum vacuum to track",
        "it, (ii) the assumed 2D dispersion, cutoff and density, and (iii) an",
        "undetermined substrate temperature below ~C/ln p. None of (i)-(iii)",
        "is derived; coherence is not established, only conditioned.",
    ]
    return "\n".join(lines)
