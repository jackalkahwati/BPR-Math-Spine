...
**Layer 2 — Structural assumptions (three, not four).** Three architectural choices determine the framework: (i) phase space = Z_p, the unique minimal finite field supporting the BPR coarse-graining; (ii) action = gradient-squared, the unique relevant operator at the 2D Gaussian CFT fixed point; (iii) particles identified with topological winding modes, motivated by topological stability under pi_2(S^2).

The boundary topology S^2 is not a fourth assumption — it is derived. Three requirements that any consistent framework must satisfy together uniquely force S^2: (a) compactness, required for a discrete particle mass spectrum; (b) orientability, required for globally consistent fermion fields; (c) trivial fundamental group pi_1 = 0, required for the absence of undetermined holonomy parameters (on a boundary with non-trivial pi_1, gauge holonomies around non-contractible loops introduce free continuous or discrete inputs not determined by p and z). By the classification of compact surfaces, the unique compact connected orientable 2-manifold with pi_1 = 0 is S^2. This result also derives the generation count: the first non-trivial eigenspace of the Laplacian on S^2 is exactly 3-dimensional (the ell = 1 spherical harmonic sector), matching the three observed fermion generations. No other compact orientable surface is simply connected.

**Layer 3 — Imported mathematical frameworks.** BPR's predictions use SO(3) representation theory, compact boson CFT, standard QFT renormalization, and the holographic principle. These are not free parameters but they are not derived from p and z — they are the mathematical consequences of the S^2 and Z_p structural choices. The precise statement of the "two integers" claim is: given the three structural assumptions above and the imported mathematical frameworks they entail, p and z together (with p determined by alpha) generate all dimensionless predictions with no remaining freedom.
