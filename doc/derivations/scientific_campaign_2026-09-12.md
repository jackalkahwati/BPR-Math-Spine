...
|---|---|---|---|
| 1 | Consistently dressed neutral Hamiltonian and density response | Complete bounded calculation; math/code review passed after two scale fixes; 53 focused and 1,079 combined tests passed; ten demos/eight algebra groups passed | Branch science/neutral-effective-response; publication pending at this commit |
| 2 | Full-model joint strong-coupling/large-volume bound and spatial scaling | Planned; not implemented | None |
...
- Module 2: full normalized density measure coordinate s=(En-E0(L)-gL)/CL. Integer-D averaging suggests ||S1||<=pi L and a first-order unitary generator remainder <=8pi*lambda*L² in C units. With source norm a=4lambda|sin(k/2)| and inherited error delta=sqrt(L)*eta, a candidate characteristic-function bound is 4delta/a+2pi*lambda*L+|t|[8pi*lambda*L²+4lambda*L/(1-4lambda*L)]. These require independent derivation/validation before publication. Combine with the existing compression theorem, prove continuity/tightness correctly, and do not infer convergence of unbounded moments.
