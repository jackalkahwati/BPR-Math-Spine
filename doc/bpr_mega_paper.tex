...
\begin{itemize}
\item $p = 104{,}729$ is the smallest prime for which the boundary formula
...
The baryon-to-photon ratio is derived from the CP-violating boundary phase
and the seesaw scale:
\begin{equation}
\eta_B = \sin(\delta_{\mathrm{CP}}) \times
  \left(\frac{\vEW}{M_{\mathrm{seesaw}}}\right)^{\!2}
= 6.83 \times 10^{-10}
\quad (\text{Planck: }6.14 \times 10^{-10},\; 11.6\%).
\end{equation}
...
22 & $\eta_B$ ($10^{-10}$) & 6.83 & 6.12 & 11.6\% & \texttt{cosmology} & Derived \\
\colrule
...
The 22-prediction particle-sector scorecard (Table~\ref{tab:master}) is
derived from zero free parameters beyond the structural inputs $(p, z)$.
The framework resolves the strong CP problem without an axion, derives the
generation number from topology, and predicts $\xi_1 = 0$ exactly from CPT
symmetry of the $\Stwo$ boundary --- a qualitatively distinct prediction
from all other quantum gravity frameworks that predict $\xi_1 \sim 1$.

The most striking new result (April 2026) is the derivation of
$\theta_{13}$ from first principles via boundary-lattice Gaussian
localization (Eq.~\ref{eq:theta13}): $\sin\theta_{13} = e^{-1}/\!\sqrt{6}$,
in agreement with Daya Bay at $+0.65\sigma$.
...
derivation; this is more constrained than a free-parameter fit but less
constrained than a first-principles derivation of $p$ itself.

\subsection{Limitations}

The $\vEW$ derivation~\eqref{eq:vew} reproduces the observed value to $1.0\%$
but not exactly; this is the single numerical tension in the consistency web.
The baryon asymmetry agrees at $11.6\%$, the least precise of the particle-sector
predictions.  The biological applications (EEG, aging) rest on the universality
of the boundary dynamics in the large-$N$ limit and are more speculative than
the particle physics sector.

\subsection{Conclusion}

Boundary Phase Resonance provides a unified framework in which all Standard
Model coupling constants, particle masses, and mixing angles emerge from the
geometry of a single discrete $\Zp$ lattice on a two-sphere boundary.  The
framework makes specific, falsifiable predictions in particle physics, quantum
gravity, and cosmology.  The coming decade of precision neutrino physics
(JUNO), axion searches (ADMX/CASPEr), and CP violation measurements
(Belle~II/LHCb) will determine whether the boundary geometry is
the correct organizing principle for physical constants.
